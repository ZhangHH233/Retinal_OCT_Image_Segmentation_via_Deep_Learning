...
def rotate(x, angle):
    """
    Rotate a tensor counter-clockwise by angle degrees.
    ...
    grid = torch.matmul(rotation_matrix, grid)
    grid = grid.t().view(h, w, 2)
    
    # 扩展到batch维度
    grid = grid.unsqueeze(0).repeat(b, 1, 1, 1)
    
    # 使用grid_sample进行采样
    output = F.grid_sample(x, grid, mode='bilinear', padding_mode='reflection', align_corners=True)
...
class EdgeAL(nn.Module):
    def __init__(self, in_channels=3, num_classes=3, ngf=64, n_downsampling=3, n_blocks=9, 
        ...
        init_conv_kwargs.update({
            'ratio_gin': ratio_gin,
            'ratio_gout': ratio_gout
        })
        
        if not downsample_conv_kwargs:
            downsample_conv_kwargs = {
                'ratio_gin': ratio_gin,
                'ratio_gout': ratio_gout
            }
            
        if not resnet_conv_kwargs:
            resnet_conv_kwargs = {
                'ratio_gin': ratio_gin,
                'ratio_gout': ratio_gout
            }
        ...
        model = [nn.ReflectionPad2d(3),
                 FFC_BN_ACT(in_channels, ngf, kernel_size=7, padding=0, 
                           norm_layer=norm_layer,
                           activation_layer=activation_layer,
                           **init_conv_kwargs)]
        ...
        model += [nn.ReflectionPad2d(3),
                  nn.Conv2d(ngf, num_classes, kernel_size=7, padding=0)]
        if add_out_act:
...
if __name__ == '__main__':
    ...
    angle = torch.tensor(10.0).to(device)  # 45度旋转
    # angle = torch.tensor(45.0).to(device)  # 45度旋转
