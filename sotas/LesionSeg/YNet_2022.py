...
class YNet_general(nn.Module):
    ...
    def __init__(self, in_channels=3, num_classes=1, init_features=32, ratio_in=0.5, ffc=True, skip_ffc=False,
                 cat_merge=True, use_input_norm=True):
...
if __name__ == '__main__':
    ...
    try:
        ...
        angle = torch.tensor(45.0).to(device)
        ...
        model = YNet_general(
            in_channels=1,
            num_classes=4,      # 改为num_classes
            init_features=32,   # 初始特征数
            ratio_in=0.5,       # FFC比例
            ffc=True,           # 使用FFC
            skip_ffc=True,      # 使用FFC跳跃连接
            cat_merge=True,     # 使用拼接合并
            use_input_norm=True # 使用输入归一化
        ).to(device)
        ...
        # 3. 测试SpectralTransform
        spectral_transform = SpectralTransform(
            in_channels=64,
            out_channels=128,   # SpectralTransform中保持out_channels
            stride=1,
            groups=1,
            enable_lfu=True
        ).to(device)
        ...
        with torch.no_grad():
            out = spectral_transform(x)
            print("\nSpectralTransform test:")
            print(f"Input shape: {x.shape}")
            print(f"Output shape: {out.shape}")
