...
class EMCAD_Net(nn.Module):
    def __init__(self,in_channels=1, num_classes=9, kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, lgag_ks=3, activation='relu', encoder='pvt_v2_b2', pretrain=False):
        super(EMCAD_Net, self).__init__()
    ...
    def forward_originalCode(self, x, mode='test'):
...
class CAB(nn.Module):
    ...
    def forward(self, x):
        ...
        # 确保输出tensor在内存中连续
        return (x * out.expand_as(x)).contiguous()
...
if __name__ == '__main__':
    ...
    model = EMCAD_Net(pretrain=False,in_channels=1,num_classes=9).cuda()
