...
class Res34_Swin_MS(nn.Module):    ###  low resolution + Multi-scale
    def __init__(self,img_size,in_channels=1,num_classes=11,zero_init_residual=False,groups = 1,width_per_group = 64): 
        ...
        #  # 修改patch embedding的输入通道数
        self.patch_embed = PatchEmbed(
            img_size=img_size, 
            patch_size=patch_size, 
            in_chans=in_channels,  # 使用传入的in_channels
            embed_dim=embed_dim
        )
    ...
    def forward(self, x: Tensor) -> Tensor: # B,C,H,W [4, 3, 512, 512]
        encoder = [] 
        ms1 = self.patch_embed(x) # B, H/2*W/2, 64
        ...
        x = x.flatten(2).transpose(1, 2) # B, H/2*W/2, 64
        
        x = x+ms1  # 1d token         
        x = self.swin_layers[0](x)    # B,  H/2*W/2, 64
        B, L, C = x.shape  # B, L=H/2*W/2, C1=64
        ms2 = self.MS2(x)  # B, C2 = C1*2, H/4, W/4
        x = x.view(B, int(np.sqrt(L)), int(np.sqrt(L)), C).permute(0,3,1, 2) # B, C1*2, H/4, W/4
        encoder.append(x)    
            
        x = self.layer2(x)    # B, C2, H/4, W/4
        x = x+ms2
        x = x.flatten(2).transpose(1, 2)
        x = self.swin_layers[1](x) # B,  H/4*W/4, C2(128)
        B, L, C = x.shape
        ms3 = self.MS3(x) # B, C2*2, H/8, W/8
...
def res34_swin_MS(img_size, num_class) :
    return Res34_Swin_MS(img_size, BasicBlock, [3, 4, 6, 3],num_classes = num_class)
...
if __name__ == "__main__":
    import torch as t
    rgb = t.randn(2, 1, 512, 512)
    net = Res34_Swin_MS(img_size=512,in_channels=1,num_classes=9)
    out = net(rgb)
    print(out.shape)
