...
"""
"""
https://github.com/yhygao/UTNet/tree/main
"""
...
class UTNet(nn.Module):
    def __init__(self, in_channels=3, num_classes=1, base_chan=32, reduce_size=8, block_list='234', 
        ...
        self.inc = [BasicBlock(in_channels, base_chan)]
        if '0' in block_list:
            self.inc.append(BasicTransBlock(base_chan, heads=num_heads[-5], 
                          dim_head=base_chan//num_heads[-5], attn_drop=attn_drop, 
                          proj_drop=proj_drop, reduce_size=reduce_size, 
                          projection=projection, rel_pos=rel_pos))
            self.up4 = up_block_trans(2*base_chan, base_chan, num_block=0, 
                                    bottleneck=bottleneck, heads=num_heads[-4], 
                                    dim_head=base_chan//num_heads[-4], 
                                    attn_drop=attn_drop, proj_drop=proj_drop, 
                                    reduce_size=reduce_size, projection=projection, 
                                    rel_pos=rel_pos)
        else:
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 测试UTNet_Encoderonly
        model_encoder = UTNet_Encoderonly(
            in_channels=1,
            num_classes=4,
            base_chan=32,
            reduce_size=8,
