...
year = {2024},
issn = {0010-4825},
doi = {https://doi.org/10.1016/j.compbiomed.2024.108784},
url = {https://www.sciencedirect.com/science/article/pii/S0010482524008692},
...
keywords = {Automatic medical image segmentation, Image-aided diagnosis, Attention mechanism, Deep learning},
...
class SegmentationNetwork(nn.Module):
    ...
    def predict(self, x, do_mirroring=True, mirror_axes=(0, 1, 2), use_sliding_window=True,
                step_size=0.5, patch_size=None, regions_class_order=None,
                use_gaussian=True, pad_border_mode="constant",
                pad_kwargs=None, all_in_gpu=False):
        if pad_kwargs is None:
            pad_kwargs = {'constant_values': 0}
        # 简化的预测函数，实际使用时可以根据需要扩展
        with torch.no_grad():
...
class CPCANet(SegmentationNetwork):
    def __init__(self, 
                 ...
                 drop_rate=0.,
                 drop_path_rate=0.2,
                 decoder_depths=[2, 2, 1],  # 解码器深度
                 conv_op=nn.Conv2d):
        """
        CPCANet: Channel and Position Collaborative Attention Network

        Args:
            in_channels (int): 输入通道数
            num_classes (int): 分割类别数
            embed_dim (int): 基础嵌入维度
            depths (List[int]): 编码器各阶段的块数
            deep_supervision (bool): 是否使用深度监督
            img_size (int): 输入图像大小
            patch_size (int): patch大小
            channelAttention_reduce (int): 通道注意力的降维比例
            drop_rate (float): dropout率
            drop_path_rate (float): drop path率
            decoder_depths (List[int]): 解码器各阶段的块数
            conv_op (nn.Module): 卷积操作类型
        """
        super(CPCANet, self).__init__()
        
        # 初始化基类属性
        self.input_shape_must_be_divisible_by = patch_size
    ...
    def forward(self, x):
        ...
        if self.do_ds:
            return seg_outputs[::-1]  # Training: return all outputs
        else:
            return seg_outputs[-1]    # Inference: return only final output
